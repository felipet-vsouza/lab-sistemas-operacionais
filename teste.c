...
void run_text_editor_job(void);
void run_terminal_job(void);
...
void run_text_editor_job(void)
{
    printf("\n\nAbrindo editor texto...");
    pid_t job_pid = fork();
    if (job_pid < 0)
    {
        failed_job(text_editor_job);
    }
    else if (job_pid == 0)
    {
    ...
    {
        start_job(text_editor_job, job_pid);
    }
}
...
void run_terminal_job(void)
{
    ...
    {
        start_job(terminal_job, job_pid);
    }
}
